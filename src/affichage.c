...
void afficher_une_carte(cartes c, int ligne) {     // créer le design d'une carte sur une même ligne
    if (ligne == 0) {
        ...
        if (c.bonus[0] != '\0')
            printf(" " OR "│" BG_GOLD TEXT_BLACK " %-3s " RESET OR "│" RESET, c.bonus);    // -3s sert à bloquer l'affichage pour éviter les décalages
        else
...
void afficher_ligne_carte(cartes *tab_cartes, int taille) {          // affiche la carte en colonne selon le nombre de lignes 
...
void afficher_accueil(void) {     // void en paramètre = sécurité contre les bugs 
    printf("\n");
    printf(ROUGE_GRAS " ╔════════════════════════════════════════╗\n" RESET);
    printf(ROUGE_GRAS " ║                                        ║\n" RESET);
...
void afficherStats(cartes *tab, int n) {
    int compteur[13] = {0};  //initialise un compteur à 0 pour chaques cartes numéros
