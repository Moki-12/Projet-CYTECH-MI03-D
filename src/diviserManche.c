...
//177 A 230
void StatsPioche (carte tab[], int n){
...
//233 A 268
void Conseil (cartes tab[], joueur joueur[], int j, int taille, int n){
                ...
                int dangerTotal = 0;
                printf("\n-------- ANALYSE DU RISQUE ---------\n");
                int debutMain = joueur[j].nb_cartes - joueur[j].nbCartesManche;
                for(int c = 0; c < joueur[j].nbCartesManche; c++){
                    int index = debutMain + c;
                    if(joueur[j].cartes[index].bonus[0] != '\0') continue;
                    int num_carte = joueur[j].cartes[index].numero;
                    int totalmain;
...
void AfficherCarte (joueur joueur[], int j){
    if (joueur[j].cartes[joueur[j].nb_cartes].bonus[0] == '\0'){
        ...
        printf ("Carte piochée : [%s]\n", joueur[j].cartes[joueur[j].nb_cartes].bonus);
    }
}
// 312 A 319
void Doublon (joueur joueur[], int j, int penalite[]){
    int debut = joueur[j].nb_cartes - joueur[j].nbCarteManche;
    if (carteExisteManche (joueur[j].cartes, debut, joueur[j].nbCarteManche, joueur[j].cartes[joueur[j].nb_cartes])){
        joueur[j].scores = 0;
    ...
    for (int i=0; i<7; i++){
        ...
        if ((*taille)==1){
            ...
            do {
                ...
                Conseil(tab, joueurs, j, &*taille, n);
