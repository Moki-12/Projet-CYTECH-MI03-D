...
typedef struct {
    char   pseudo[100];
    int    scores;
    cartes cartes[500]; //Taille augmentée pour éviter un overflow 
    int    nb_cartes;
    int    score_total;
    int    nbCartesManche;
    int    score_pot;
...
void afficherScoresManche(joueur *joueurs, int nb_joueur, int TAILLE) {
    printf("\n " MAGENTA_GRAS "SCORES DE LA MANCHE" RESET "\n");
    
    // 1. Affichage des scores de chaque joueur pour cette manche
    for (int j = 0; j < nb_joueur; j++) {
        printf("\n " BLANC_GRAS "★ %s" RESET, joueurs[j].pseudo);
        printf("\n Score manche : %d", joueurs[j].score_pot);
    ...
    // 2. Recherche du score total maximum actuel
    int maxScore = -1;
    for (int j = 0; j < nb_joueur; j++) {
    ...
    // 3. Compter combien de joueurs ont ce score max
    int nbLeaders = 0;
    for (int j = 0; j < nb_joueur; j++) {
        if (joueurs[j].score_total == maxScore) {
            nbLeaders++;
    ...
    if (nbLeaders > 1) {
        printf(JAUNE_GRAS " Egalité avec %d points entre " RESET , maxScore);
        int premier = 1;
        for (int j = 0; j < nb_joueur; j++) {
            if (joueurs[j].score_total == maxScore) {
                if (!premier) {
                ...
                printf(JAUNE_GRAS "%s" RESET, joueurs[j].pseudo);
                premier = 0;
            }
        }
        printf("\n");
    } else {
        // Un seul leader unique
        for (int j = 0; j < nb_joueur; j++) {
            if (joueurs[j].score_total == maxScore) {
                printf(JAUNE_GRAS " %s est seul en tête avec %d points !" RESET "\n", joueurs[j].pseudo, maxScore);
                break;
            }
        }
    }
...
int main() {
    ...
    do {
        ...
        if (fin != 0 && TAILLE > 0)
            afficherScoresManche(joueurs, nb_joueur, TAILLE);
